...
type Object3DLike = { position: Vector3 };
type Vec3Obj3D = Object3DLike | Vector3;

const _plane = new Plane();
const _temp = new Vector3();

/** @internal */
export function getPosition(object: Vec3Obj3D): Vector3 {
  return (object as Object3DLike).position ?? object as Vector3;
}

/** @internal */ // TODO capire se tenere o eliminare
export function getPositions(objects: Vec3Obj3D[]): Vector3[] {
  for (let i = 0, l = objects.length; i < l; i++) {
    objects[i] = getPosition(objects[i]);
  }
  return objects as Vector3[];
}
 ...
 */
export function getSceneIntersection(ray: Ray, camera: Camera, distance: number): Vector3 {
