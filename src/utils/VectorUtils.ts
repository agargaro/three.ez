...
type Vector3Object3D = Vector3 | Object3D;
...
export class VectorUtils {
  ...
  public static haveSameDirection(v1: Vector3, v2: Vector3, tolerance = 0.01): boolean {
    return TEMP[0].copy(v1).normalize().dot(TEMP[1].copy(v2).normalize()) > 1 - tolerance;
  }

  public static haveOppositeDirection(v1: Vector3, v2: Vector3, tolerance = 0.01): boolean {
  ...
  public static perpendicularByPoints(p1: Vector3Object3D, p2: Vector3Object3D, target = new Vector3(), normal = this.DEFAULT_NORMAL) {
    const [p1c, p2c] = this.getPositionsFromObject3D([p1, p2]);
    return target.crossVectors(TEMP[0].subVectors(p1c, p2c), normal);
  }

  public static perpendicularByPointsSigned(p1: Vector3Object3D, p2: Vector3Object3D, referencePoint: Vector3Object3D, target = new Vector3(), normal = this.DEFAULT_NORMAL) {
    const [p1c, p2c, r] = this.getPositionsFromObject3D([p1, p2, referencePoint]);
  ...
  public static bisector(v1: Vector3, v2: Vector3, target = new Vector3(), normal = this.DEFAULT_NORMAL) {
    TEMP[0].copy(v1).normalize();
    TEMP[1].copy(v2).normalize();
    return target.addVectors(TEMP[0], TEMP[1]).normalize();
  }

  public static bisectorByPoints(p1: Vector3Object3D, p2: Vector3Object3D, center: Vector3Object3D, target = new Vector3(), normal = this.DEFAULT_NORMAL) {
    const [p1c, p2c, c] = this.getPositionsFromObject3D([p1, p2, center]);
    return this.bisector(TEMP[2].subVectors(p1c, c), TEMP[3].subVectors(p2c, c), target, normal);
  }

  public static arePointsOnSameSideByPoints(p1: Vector3Object3D, p2: Vector3Object3D, points: Vector3Object3D[], normal = this.DEFAULT_NORMAL): boolean {
  ...
  public static arePointsOnSameSide(origin: Vector3Object3D, dir: Vector3, points: Vector3Object3D[], normal = this.DEFAULT_NORMAL): boolean {
  ...
  public static signedAngleFromOrigin(a: Vector3, b: Vector3, normal = this.DEFAULT_NORMAL): number {
    return Math.atan2(TEMP[0].crossVectors(a, b).dot(normal), a.dot(b));
  }

  public static signedAngleByPoints(p1: Vector3Object3D, p2: Vector3Object3D, center: Vector3Object3D, normal = this.DEFAULT_NORMAL): number {
    const [p1c, p2c, c] = this.getPositionsFromObject3D([p1, p2, center]);
  ...
  public static projectOnLine(point: Vector3Object3D, l1: Vector3Object3D, l2: Vector3Object3D, target = new Vector3()): Vector3 {
  ...
  // https://paulbourke.net/geometry/pointlineplane/
  public static lineIntersection3D(p1: Vector3Object3D, p2: Vector3Object3D, p3: Vector3Object3D, p4: Vector3Object3D, target = new Vector3()): Vector3 {
